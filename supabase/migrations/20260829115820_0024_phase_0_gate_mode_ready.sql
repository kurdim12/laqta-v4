-- 0024_phase_0_gate_mode_ready.sql
--
-- 0025 will make guest-code minting obey the event's guest mode structurally. The Phase 0
-- gate predates modes as enforcement: it mints photo-bound codes on a gate event created
-- with the default mode (wall_only), which the coming rule rightly refuses. So the gate is
-- restated FIRST, in its own migration: every check is unchanged, and only the setup line
-- adapts - the gate event now declares code_per_shot, the mode its minting checks always
-- assumed. Applied against today's mode-blind mint this is a no-op in behaviour, which is
-- exactly why it can land ahead of the enforcement and be proven green on both sides of it.
--
...
begin
  ...
  -- Restated by 0024: gate-alpha is now created in code_per_shot mode, because minting a
  -- photo-bound code on a wall_only event is precisely what 0024 makes impossible.
  insert into events (slug, name, status, telemetry_cap_per_device_hour, upload_ttl_minutes, guest_mode)
  values ('gate-alpha', 'Gate Alpha', 'live', 10, 1, 'code_per_shot') returning id into a;
...
begin
  ...
  if v_fails > 0 then
    raise exception 'gate suite red after 0024: % failing check(s)', v_fails;
  end if;
end
$$;
